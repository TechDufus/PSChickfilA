...
Describe "Given: PSChickfilA Manifest" {
    Context "When: Manifest is being used" {
        $script:ThisRoot = Split-Path $PSScriptRoot -Parent
        $ManifestInfo = Test-ModuleManifest -Path ([System.IO.Path]::Combine($script:MyRoot, 'PSChickfilA.psd1'))
        ...
        It 'Then: should have the correct RootModule' {
            ...
            $RootModule | Should -Match "PSChickfilA.psm1"
        }
    }
}